...
export const Table: React.FC<TableProps> = ({ columns, data, buttonActions }) => {
          ...
          <thead>
            <tr className="text-left">
              {column.map((col: Columns, index: number) => (
                <th key={index} className="p-4 border-gray-200 border-b text-warm-black">
                  {getCaps(col.header ?? '', col.label ?? '')}
                </th>
              ))}
              <th className="p-4 border-gray-200 border-b text-warm-black">Ações</th>
            </tr>
          </thead>
            ...
            {rows.map((row: Rows, rowIndex: number) => (
              <tr className="text-left" key={rowIndex}>
                {column.map((col: Columns, colIndex: number) => (
                  <td key={colIndex} className="p-4 border-gray-200 border-b text-deep-space-sparkle">
                    <label>{row[col.label as keyof typeof row.name]}</label>
                  </td>
                ))}
                <td className="p-4 border-gray-200 border-b text-warm-black">{buttonActions?.(row.id)}</td>
              </tr>
            ))}
          </tbody>
        </table>
